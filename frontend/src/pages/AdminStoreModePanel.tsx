...
import { CustomerDashboardData, GameDetails, Order, Review, UserLibrary, Wishlist } from '../types';
...
const AdminStoreModePanel: React.FC<AdminStoreModePanelProps> = ({ onBackToAdmin }) => {
  ...
  const [catalog, setCatalog] = useState<GameDetails[]>([]);
  ...
  const handlePurchase = (itemType: 'game' | 'dlc' | 'edition', itemId: number | string) => {
    if (isOwned(itemType, itemId)) {
      setError(`You already own this ${itemType}.`);
      setMessage(null);
      return;
    }
  ...
  const handleAddWishlist = (gameId: number | string) => {
    runAction(`wishlist-add-${gameId}`, async () => {
  ...
  const handleRemoveWishlist = (gameId: number | string) => {
    runAction(`wishlist-remove-${gameId}`, async () => {
  ...
  const handleAddFunds = () => {
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      setError('Enter a wallet amount greater than zero.');
      setMessage(null);
      return;
    }

    runAction('wallet-add-funds', async () => {
      await walletApi.addFunds(amount);
      setWalletAmount('');
      setMessage(`${formatPrice(amount)} added to wallet.`);
    });
  };
  ...
  const handleSaveReview = () => {
    ...
    runAction(`review-save-${selectedGame.game.gameId}`, async () => {
      if (currentUserReview) {
        await reviewsApi.update(currentUserReview.reviewId, { rating, comment });
        setMessage('Review updated.');
      } else {
        await reviewsApi.create({ gameId: selectedGame.game.gameId, rating, comment });
