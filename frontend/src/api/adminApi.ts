...
import { Category, Game, GameCategory, GamePlatform, Platform, User } from '../types';
...
export const adminPlatformsApi = {
  ...
  update: async (id: string | number, data: Record<string, any>): Promise<Platform | null> => {
    const result = await apolloClient.mutate<any>({
      ...
      variables: { platformId: toInt(id), updatePlatformInput: withGraphqlPlatformName(data) },
    });
...
export const adminCategoriesApi = {
  ...
  update: async (id: string | number, data: Record<string, any>): Promise<Category> => {
    const result = await apolloClient.mutate<any>({
      ...
      variables: { categoryId: toInt(id), updateCategoryInput: categoryInput(data) },
    });
...
export const adminGameCategoriesApi = {
  ...
  create: async (data: { gameId: string | number; categoryId: string | number }): Promise<GameCategory> => {
    const result = await apolloClient.mutate<any>({
      mutation: ADD_CATEGORY_TO_GAME,
      variables: { gameId: toInt(data.gameId), categoryId: toInt(data.categoryId) },
    });
  ...
  update: async (
    ...
    return adminGameCategoriesApi.create({ gameId, categoryId: data.newCategoryId });
  },
  remove: async (gameId: string | number, categoryId: string | number): Promise<boolean> => {
    const { data } = await apolloClient.mutate<any>({
      mutation: REMOVE_CATEGORY_FROM_GAME,
      variables: { gameId: toInt(gameId), categoryId: toInt(categoryId) },
    });
...
export const adminGamePlatformsApi = {
  ...
  create: async (data: { gameId: string | number; platformId: string | number }): Promise<GamePlatform> => {
    const result = await apolloClient.mutate<any>({
      mutation: ADD_PLATFORM_TO_GAME,
      variables: { gameId: toInt(data.gameId), platformId: toInt(data.platformId) },
    });
  ...
  update: async (
    ...
    return adminGamePlatformsApi.create({ gameId, platformId: data.newPlatformId });
  },
  remove: async (gameId: string | number, platformId: string | number): Promise<boolean> => {
    const { data } = await apolloClient.mutate<any>({
      mutation: REMOVE_PLATFORM_FROM_GAME,
      variables: { gameId: toInt(gameId), platformId: toInt(platformId) },
    });
