...
export class PlatformService {
  ...
  async createPlatform(createDto: CreatePlatformDto): Promise<Platform> {
    const existing = await this.platformRepository.findByName(
      createDto.platformName,
    );

    if (existing) {
      throw new ConflictException(
        `Platform with name '${createDto.platformName}' already exists`,
      );
    }
  ...
  async updatePlatform(
    ...
    const existing = await this.platformRepository.findByName(
      updateDto.platformName,
    );

    if (existing && existing.platformId !== id) {
      throw new ConflictException(
        `Platform with name '${updateDto.platformName}' already exists`,
      );
    }
