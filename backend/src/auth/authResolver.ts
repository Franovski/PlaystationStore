...
import { ForgotPasswordDto, ResetPasswordDto, VerifyTotpDto } from './dto/auth-additional';
...
export class AuthResolver {
  ...
  @Mutation(() => Boolean)
  async forgotPassword(@Args('forgotPasswordInput') forgotPasswordDto: ForgotPasswordDto) {
    await this.passwordResetService.forgotPassword(forgotPasswordDto.email);
    return true;
  }

  @Mutation(() => Boolean)
  async resetPassword(@Args('resetPasswordInput') resetPasswordDto: ResetPasswordDto) {
  ...
  async verifyTotp(@Args('verifyTotpInput') verifyTotpDto: VerifyTotpDto) {
    return this.authService.verifyTotpAndLogin(verifyTotpDto.tempToken, verifyTotpDto.code);
  }
}
