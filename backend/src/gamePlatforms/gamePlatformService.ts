...
export class GamePlatformService {
  ...
  async linkGameAndPlatform(dto: AddGamePlatformDto): Promise<GamePlatform> {
    ...
    if (existing) {
      throw new ConflictException(
        `Game ID ${dto.gameId} is already linked with Platform ID ${dto.platformId}`,
      );
    }
  ...
  async unlinkGameAndPlatform(
    ...
    if (!existing) {
      throw new NotFoundException(
        `Link between Game ID ${gameId} and Platform ID ${platformId} not found`,
      );
    }
  ...
  async getGamesByPlatform(platformId: number) {
    ...
    const mappings =
      await this.gamePlatformRepository.findByPlatform(platformId);
